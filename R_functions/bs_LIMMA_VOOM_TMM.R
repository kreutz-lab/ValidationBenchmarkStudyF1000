...
#
# Example:
# res <- bs_limma_voom(count=d[[i]][[j]]$counts[1:300,],meta=d[[i]][[j]]$meta)


bs_limma_voom <- function(counts,meta,groupVariable="condition",fileName=NULL){
  
  if(inherits(counts,"data.frame"))
    counts <- as.matrix(counts)
  
  if(!all(!is.na(counts)))
    warning("bs_limma_voom: NAs in counts.")
  if(sum(counts<0,na.rm=T)>0)
    warning("bs_limma_voom: Counts have negative values.")
...
#  summary_upper_quartile <- summary(Upper_Quartile_norm_test$samples$norm.factors)[3]
  summary_upper_quartile <- median(Upper_Quartile_norm_test$samples$norm.factors) # CK
  if(is.na(summary_upper_quartile) | is.infinite(summary_upper_quartile)){
    ...
    DGE_LIST_Norm <- edgeR::calcNormFactors(DGE_LIST, method = "TMM", refColumn = Ref_col)
  ...
  mm <- model.matrix(as.formula(paste0("~",groupVariable)), meta)
