# zinbwave 
#
#   Calling zinbwave for doing differential abundance analysis
#
...
#
# Example:
# res <- bs_zinbWave(count=d[[i]][[j]]$counts[1:300,],meta=d[[i]][[j]]$meta)

bs_zinbWave <- function(counts,meta,design=as.formula("~condition"),fileName=NULL,maxRunTimeInMin=NULL,maxFeatures=Inf){
  
  if(!inherits(counts,"matrix"))
    counts <- as.matrix(counts)
  
  if(maxFeatures<Inf){
    cat("maxFeatures = ",maxFeatures,"\n")
  }
  count <- counts[1:min(maxFeatures,nrow(counts)),]
  
  if(!all(!is.na(counts)))
    warning("bs_zinbWave: NAs in counts.")
  if(sum(counts<0,na.rm=T)>0)
    warning("bs_zinbWave: Counts have negative values.")
  
  condition <- meta$condition
  design <- model.matrix(design, data = meta)
  
  bs_zinbWave_core <- function(counts, meta, design,min_non_zero_cells) {
    library(BiocParallel)
    ...
    se <- SummarizedExperiment::SummarizedExperiment(
      assays = list(counts = counts),  # Log-normalized counts
      colData = meta
    )

    #old_seed <- .Random.seed
    #set.seed(61217) # seed from distinct::distinct_test docu
    res <- zinbwave::zinbwave(Y=se, X = design, BPPARAM=MulticoreParam(20))
  ...
  if(is.null(maxRunTimeInMin)){
    out <- bs_zinbWave_core(counts,meta,design)
    return(out)
