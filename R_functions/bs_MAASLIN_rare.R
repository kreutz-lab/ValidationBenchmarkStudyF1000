...
#
# Example:
# res <- bs_Maaslin2_rare(counts,meta)
# res <- bs_Maaslin2(count=d[[i]][[j]]$counts[1:300,],meta=d[[i]][[j]]$meta)


bs_Maaslin2_rare <- function(counts,meta,groupVariable="condition",fileName=NULL,min_prevalence=0){
  ...
  ## Rarefy the data
  tmp <- bs_doRarefy(counts,groupings)
