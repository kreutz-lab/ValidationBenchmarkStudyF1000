###############################################
# Kreutz-lab downloaded major parts of this script from the following source on June, 18th 2024:
# https://raw.githubusercontent.com/nearinj/Comparison_of_DA_microbiome_methods/main/Pipeline_scripts/Ancom2_Script/ancom_v2.1.R
###############################################


# This function implements the call of ANCOM corresponding to the implementation in Run_ANCOM by Nearing et al.
bs_ancom_Nearing <- function(counts, meta, groupVariable="condition",fileName=NULL,p_adj_method="BH"){
  
  meta$Sample <- rownames(meta)
  prepro <- feature_table_pre_process(counts, meta, sample_var = "Sample", lib_cut = 1000, neg_lb=FALSE)
  feature_table <- prepro$feature_table
  metadata <- prepro$meta_data
  struc_zero <- prepro$structure_zeros
  
  #run ancom
  main_var <- groupVariable #colnames(groupings)[1]
  ...
  pvals <- array(NA,nrow(counts)) 
  padj <- array(NA,nrow(counts)) 
  lfc <- array(NA,nrow(counts)) 
  ind <- as.numeric(rownames(feature_table))
  pvals[ind] <- rowMedians(res$p_data)
  
  # medians are always more centered than raw p-values
  # By the following, I partly rescale to the original distribution of p_data
  tmp <- res$p_data
  diag(tmp) <- NA
  tmp <- tmp[!is.na(tmp)]
  rf <- order(pvals[ind])
  pvals[ind[rf]] <- quantile(tmp,seq(0,1,length.out=2*length(ind)+1))[seq(2,2*length(ind),by=2)]
  
  padj[ind] <- p.adjust(pvals[ind],method = "BH")
    
  
  out <- list(pvalue=pvals, logFoldChange=lfc, padj=padj, result=res$out)
  return(out)
...
###############################################
# The following has been downloaded:
###############################################
...
ANCOM = function(feature_table, meta_data, struc_zero = NULL, main_var, p_adj_method = "BH", 
                 alpha = 0.05, adj_formula = NULL, rand_formula = NULL, doPlot=F, ...){
  ...
  if(doPlot){# added by ck
    ...
    fig <- NULL# added by ck
  }# added by ck
  
#  res = list(out = out, fig = fig)
  res = list(out = out, fig = fig, q_data=q_data, p_data=p_data) # added by ck
  return(res)
}
