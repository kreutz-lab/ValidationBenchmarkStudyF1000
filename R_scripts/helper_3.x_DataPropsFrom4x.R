...
for(s in 1:length(simulators)){
  ...
  analysis <- analyses[s]
  partReg <- partRegs[s]
  folder3 <- bs_getPath(analysis,simulator, partReg = partReg) # folder for saving data props
  folder4 <- bs_getPath(analysis+1,simulator, nearing = nearing ,partReg = partReg) # folder for reading data_to_compare
