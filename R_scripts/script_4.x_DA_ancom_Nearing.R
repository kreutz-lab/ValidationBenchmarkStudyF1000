...
print("script_4.x_DA_ancom_Nearing.R started ...")
if(!dir.exists(dataFolder)){
  ...
  data_to_compare <- foreach(D=data_to_compare) %dopar% {
    ...
    tryCatch({
      ...
      if(doDA){  # DA analysis, time consuming
        #      D <- bs_DA(D,whichMethods = "edgeR",maxFeatures = 1000, parallelMode = T, maxRunTime = 60)
        D <- bs_DA(D, parallelMode = T, doSlowMethods = T, whichMethods = "ancom_nearing", keepTestResult=T, maxRunTime = 60)
        #D <- bs_DA_linux(D,whichMethods = "glmmTMB",maxFeatures = 5000, maxRunTime = 60) # linux, without microbiomeMarker package
  ...
  print("script_4.x_DA_ancom_Nearing.R finished :-) ")
}
