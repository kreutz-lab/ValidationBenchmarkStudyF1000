...
d <- bs_decompress(readRDS(paste0("../Results/2.2_sparseDOSSA/data_to_compare.RDS")))
...
params <- foreach(D = d, .packages = c('digest')) %dopar% {
  tryCatch({
    ...
    countsInGroups <- list()
    countsInGroups[[1]] <- D$original$counts
    names(countsInGroups) <- "allSamples"
    ...
    for(i in 1:length(countsInGroups)){
      ...
      save(SparseDOSSA2_fit,D,file=paste0("../Results/1.2_sparseDOSSA/estimateSparseDossa_partReg_foreach",D$index,as.character(cond),".Rdata"))
    ...
    try(save(fits,D,file=paste0("Error_in_estimateSparseDossa_partReg_foreach",D$index,".Rdata")))
    cat("Error :",conditionMessage(e),"\n")
    cat("Error :",conditionMessage(e),"\n",file="estimateSparseDossa_partReg_foreach.log",append = T)
...
saveRDS(params,file="params_tmp.RDS")

inFolder <- "../Results/1.2_sparseDOSSA/"
outFile <- paste0("../Results/1.2_sparseDOSSA/paramsSparseDossa2_partReg",msb.info()[["date"]],".RDS")
